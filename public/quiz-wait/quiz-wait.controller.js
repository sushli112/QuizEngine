...
const populateScoreInGamePage = (playerScoreData) => {
    ...
    if (playerScoreData) {
        playerScoreData.forEach((player) => {
            scorePageHTML += `<div class="col-sm">${player.player} : ${player.score}</div>`;
        });
        if (document.getElementById("playerScoreInfo")) {
...
const populateGamePage = (socketCustom) => {
    ...
    document.getElementById("quizHeader").innerHTML = "QUIZ PAGE";
    const gamePageHTML = `<div class="container container-game">
		<div class="row clearfix">
          <div class="col-sm-12">
          <div class="col-md-12"><p class="text-center">Time Left:&nbsp<span class="counts-left" style="color:red"></span></p></div>
          <div id="myProgress"><div id="myBar"></div></div></div>
        </div>
        <p class="d-none">Score: <span id="answerscore" style="color:red">0</span></p>
        <div class="container">
...
const dynamicProgressBarQuestion = (socketCustom) => {
    ...
    function frame() {
        if (width >= 100) {
            ...
            $(".counts-left").html(10-(Math.round(secstrack/10)));
        } 
    }
}
...
const updateQues = (nextques) => {
    ...
    $("button").css({"visibility":"hidden","transition": "visibility 0s, opacity 0.5s linear"});
